...
async function seed(){
  try{
    ...
    const r=await fetch('./data/food-library.json');
...
function open(){
  let overlay=document.getElementById('bane-food-overlay');
  if(!overlay){overlay=document.createElement('div');overlay.id='bane-food-overlay';document.body.appendChild(overlay);}
  render();
}
function render(filter=''){
 const overlay=document.getElementById('bane-food-overlay');
 const list=library.filter(f=>f.name.toLowerCase().includes(filter.toLowerCase()));
 overlay.innerHTML=`<div class="food-modal"><div class="food-head"><div><small>PERSONAL LIBRARY</small><h2>Food Library</h2></div><button id="food-close">Close</button></div><p class="muted">Tap a food to add it to today's log. Amount is ${list.some(f=>f.servingUnit==='oz')?'weight for weight-based foods or servings for the rest': 'servings'}.</p><input id="food-search" placeholder="Search your foods…" value="${esc(filter)}"><div class="food-list">${list.map(f=>`<button class="food-card" data-food="${esc(f.id)}"><div><b>${esc(f.name)}</b><span>${esc(f.defaultServing)}</span></div><strong>${f.caloriesPerUnit} cal · ${f.proteinPerUnit}g P · ${Math.max(0,f.carbsPerUnit-f.fiberPerUnit)}g net C</strong></button>`).join('')||'<p>No foods found.</p>'}</div><div class="food-actions"><button id="food-import-logged">Add logged foods to library</button><button id="food-new" class="primary">Create Food</button></div><div id="food-form"></div></div>`;
